...
async function init() {
  try {
    // Kinukuha natin ang data mula sa TMDB gamit ang tamang parameters para sa bawat niche section mo
    const [trd, anime, fil, kd, kp, kids] = await Promise.all([
      ...
      fetch(`${BASE_URL}/discover/movie?api_key=${API_KEY}&region=PH&with_original_language=tl`).then(r => r.json()), // Mas tumpak na filter para sa Tagalog/Vivamax vibe
    ...
    // Dito pinapagana at pinapakita ang bawat nakuha nating pelikula sa kani-kanilang scroller rows
...
async function viewAll(containerId) {
    ...
    if (containerId === "trending-today") {
        ...
        url = `${BASE_URL}/discover/movie?api_key=${API_KEY}&region=PH&with_original_language=tl`;
