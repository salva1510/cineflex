...
// --- NETFLIX INTRO SOUND ENGINE (UPDATED) ---
function startAppWithSound() {
    const sound = document.getElementById("intro-sound");
    const splash = document.getElementById("splash-overlay");
    
    if (sound) {
        // Hinihilingan ang browser na mag-play sa user interaction event
        sound.play()
...
function updateServerTabsUI() {
    ...
    if (!btn1 || !btn2) return; // Iniiwasan ang error kung wala pang element sa view
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // SIGURADUHING KONEKTADO ANG INTRO OVERLAY ENGINE SA EVENT LISTENERS
