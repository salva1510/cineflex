...
const SERVER_2_URL = "https://zxcstream.xyz"; // Siguraduhing may fallback or palitan ito kung may server 2
...
async function init() {
  try {
    ...
    updateContinueUI();
    setupInfiniteScroll(); // Ligtas na initialization para sa scroll listener
  } catch (err) { 
...
// --- BAGONG LIMITLESS VIEW ALL FUNCTION ---
async function viewAll(containerId) {
    ...
    if (containerId === "trending-today") url = `${BASE_URL}/trending/all/day?api_key=${API_KEY}`;
    else if (containerId === "marvel-list") url = `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_companies=420&sort_by=release_date.desc`;
    else if (containerId === "pinoy-action-list") url = `${BASE_URL}/discover/movie?api_key=${API_KEY}&region=PH&with_
