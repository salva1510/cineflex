...
function triggerPopUnder() {
  // IPAD AIR 2 FIX: Pinoprotektahan ang execution para hindi mag-crash ang browser kung sakaling i-block ng Safari ang script injection
  try {
...
async function enterCinemaMode() {
  ...
  try {
    ...
    // IPAD AIR 2 FIX: Mas ligtas na pagsusuri bago tawagin ang Screen Orientation para iwas freeze sa iOS Safari
...
// Fullscreen listeners na katugma pati ang lumang WebKit ng Apple
...
async function init() {
  try {
    const [trd, marvel, anime, fil, kd, kp, kids, pinoyAction, dramabox, netflixMovies, cocomelonData] = await Promise.all([
      ...
      // Cocomelon + Kids alternative list para marami ang lumabas sa row
...
async function playTrailer() {
    ...
    try {
        ...
        if (trailerItem && trailerItem.key) {
            ...
            // IPAD AIR 2 FIX: I-delay ang ads para hindi masabayan at maharangan ng Safari ang playback event
            setTimeout(triggerPopUnder, 1200);
        } else {
            alert("Paumanhin, walang available na trailer para sa palabas na ito sa ngayon.");
        }
    } catch (err) {
        console.error("Trailer System Error:", err);
        alert("Hindi ma-load ang trailer. Subukan muli mamaya.");
    }
}
...
function viewWatchlist() {
    ...
    if (watchlist.length === 0) {
        font.innerHTML = `<div style="color:#aaa; text-align:center; width:100%; padding:40px; font-size:1rem;"><i class="fa-solid fa-heart-broken" style="font-size:2rem; margin-bottom:10px; display:block; color:#555;"></i>Walang laman ang iyong My List.</div>`;
        return;
    }
...
function triggerDownload() {
    ...
    if (type === 'tv') {
        downloadUrl = `https://zxcstream.xyz/download/tv/${movieId}/${season}/${episode}`;
    } else {
        downloadUrl = `https://zxcstream.xyz/download/movie/${movieId}`;
    }
...
// --- CORE VIDEO STREAM CONFIG & IPAD FALLBACK ENGINE ---
function updateVideoSource() {
    ...
    // IPAD AIR 2 DETECTOR ENGINE: Sinisiyasat kung ang browser ay isang iOS Safari o iPad multi-touch device
    const isAppleDevice = /iPad|iPhone|iPod/.test(navigator.userAgent) || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
    
    if (isAppleDevice) {
        // Isara ang black box loader sa modal para malinis tingnan
        const playerContainer = document.getElementById("modal-player-container");
        if (playerContainer) playerContainer.style.display = "none";
        
        // CRITICAL FALLBACK: Imbes na iframe na hinarang ng Apple, bubuksan ang link sa bagong bintana para mapwersa ang Native iOS player na mag-stream
        window.open(finalUrl, '_blank');
    } else {
        // Normal na iframe rendering para sa mga Android, PC, at Smart TVs
...
function switchServer(serverNum) {
    ...
    updateServerTabsUI();
    updateVideoSource();
    setTimeout(triggerPopUnder, 1000);
}
...
function playSpecificEpisode(epNum, element) {
    ...
    // IPAD AIR 2 FIX: Ginamitan ng Buffer timeout para makalusot ang playback event bago tumawag ng ads script
...
function startPlayback() {
    ...
    // IPAD AIR 2 FIX: Buffer delay laban sa iframe loading restriction
    setTimeout(triggerPopUnder, 1500);
}
