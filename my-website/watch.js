(() => {
  ...
  let season=Number(q.get('season')||1), episode=Number(q.get('episode')||1), isVip=false, adReady=false, adTimer=null, totalEpisodes=0;
  ...
  let lastEpisodeAdAt=0;
  function setAdTimer(){clearTimeout(adTimer);adReady=!isVip}
  function triggerFreeAd(){
    if(isVip)return false;
    const now=Date.now();
    if(now-lastEpisodeAdAt<1500)return false;
    lastEpisodeAdAt=now;
    const old=document.getElementById('cineflex-watch-play-popad');if(old)old.remove();
    const s=document.createElement('script');s.id='cineflex-watch-play-popad';s.src=`https://bashsecret.com/03/53/7d/03537deb3b1a6012bf51de011865aed1.js?cf_episode=${now}`;s.async=true;s.onload=s.onerror=()=>setTimeout(()=>s.remove(),15000);document.body.appendChild(s);return true
  }
  ...
  $('prevEpisodeBtn').onclick=()=>selectEpisode(episode-1);
  $('nextEpisodeActionBtn').onclick=()=>selectEpisode(episode+1);
  ...
  document.addEventListener('keydown',event=>{
    ...
    if(key==='n'&&type==='tv'){event.preventDefault();selectEpisode(episode+1)}
    if(key==='p'&&type==='tv'){event.preventDefault();selectEpisode(episode-1)}
  });
  renderServers();
