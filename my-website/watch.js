(() => {
  ...
  const SERVERS=[
    {id:'vidsrcsbs',name:'vidsrc.sbs',label:'Server 1',note:'Primary',movie:id=>`https://vidsrc.sbs/embed/movie/${id}`,tv:(id,s,e)=>`https://vidsrc.sbs/embed/tv/${id}/${s}/${e}`},
    {id:'vidlink',name:'vidlink.pro',label:'Server 2',note:'Backup 1',movie:id=>`https://vidlink.pro/movie/${id}`,tv:(id,s,e)=>`https://vidlink.pro/tv/${id}/${s}/${e}`},
    {id:'peach',name:'peachify.pro',label:'Server 3',note:'Backup 2',movie:id=>`https://peachify.pro/embed/movie/${id}`,tv:(id,s,e)=>`https://peachify.pro/embed/tv/${id}/${s}/${e}`},
    {id:'oneembed',name:'1embed.cc',label:'Server 4',note:'Backup 3',movie:id=>`https://1embed.cc/embed/movie/${id}`,tv:(id,s,e)=>`https://1embed.cc/embed/tv/${id}/${s}/${e}`},
    {id:'vidsrcme',name:'vidsrcme.ru',label:'Server 5',note:'Backup 4',movie:id=>`https://vidsrcme.ru/embed/movie/${id}`,tv:(id,s,e)=>`https://vidsrcme.ru/embed/tv/${id}/${s}/${e}`},
    {id:'vidsrcto',name:'vidsrc.to',label:'Server 6',note:'Backup 5',movie:id=>`https://vidsrc.to/embed/movie/${id}`,tv:(id,s,e)=>`https://vidsrc.to/embed/tv/${id}/${s}/${e}`},
    {id:'vsembed',name:'vsembed.ru',label:'Server 7',note:'Backup 6',movie:id=>`https://vsembed.ru/embed/movie/${id}`,tv:(id,s,e)=>`https://vsembed.ru/embed/tv/${id}/${s}/${e}`},
    {id:'twoembed',name:'2embed.cc',label:'Server 8',note:'Backup 7',movie:id=>`https://2embed.cc/embed/${id}`,tv:(id,s,e)=>`https://2embed.cc/embedtv/${id}&s=${s}&e=${e}`},
    {id:'moviepire',name:'video.moviepire.co',label:'Server 9',note:'Backup 8',movie:id=>`https://video.moviepire.co/embed/movie/${id}`,tv:(id,s,e)=>`https://video.moviepire.co/embed/tv/${id}/${s}/${e}`},
    {id:'vidfast',name:'vidfast.vc',label:'Server 10',note:'Backup 9',movie:id=>`https://vidfast.vc/movie/${id}?autoPlay=true`,tv:(id,s,e)=>`https://vidfast.vc/tv/${id}/${s}/${e}?autoPlay=true`},
    {id:'zxc',name:'zxcstream.xyz',label:'Server 11',note:'Backup 10',movie:id=>`https://zxcstream.xyz/player/movie/${id}`,tv:(id,s,e)=>`https://zxcstream.xyz/player/tv/${id}/${s}/${e}`},
    {id:'vidnest',name:'vidnest.fun',label:'Server 12',note:'Backup 11',movie:id=>`https://vidnest.fun/movie/${id}`,tv:(id,s,e)=>`https://vidnest.fun/tv/${id}/${s}/${e}`}
  ];
  if(localStorage.getItem('cineflex_server_order_build')!=='310.1'){
    localStorage.setItem('cineflex_preferred_server','vidsrcsbs');
    localStorage.setItem('cineflex_server_order_build','310.1');
  }
  let season=Number(q.get('season')||1), episode=Number(q.get('episode')||1), isVip=false, membershipResolved=false, adReady=false, adTimer=null, totalEpisodes=0, episodeRuntime=0, autoNextTimer=null, autoNextCountdown=null;
  ...
  try{const saved=JSON.parse(localStorage.getItem('cineflex_server_latency')||'{}');Object.entries(saved).forEach(([key,value])=>{if(Number.isFinite(Number(value)))serverLatency.set(key,Number(value))})}catch(e){}
  ...
  function saveLatency(){
    try{localStorage.setItem('cineflex_server_latency',JSON.stringify(Object.fromEntries(serverLatency)))}catch(e){}
  }
  function fastestServerIndex(){
    const ranked=SERVERS.map((server,index)=>({index,latency:serverLatency.get(server.id)})).filter(x=>Number.isFinite(x.latency)).sort((a,b)=>a.latency-b.latency);
    return ranked.length?ranked[0].index:currentServerIndex;
  }
  function useFastestServer(){
    const index=fastestServerIndex();
    const known=Number.isFinite(serverLatency.get(SERVERS[index].id));
    fallbackAttempts=0;
    switchServer(index,true);
    toast(known?`Fastest saved server: ${SERVERS[index].name}`:'Testing your preferred server first');
  }
  ...
  function cancelAutoNext(){clearTimeout(autoNextTimer);clearInterval(autoNextCountdown);autoNextTimer=null;autoNextCountdown=null;const o=$('cf312NextOverlay');if(o){o.hidden=true;o.setAttribute('aria-hidden','true');o.style.display='none';}}
  function showAutoNext(){
    if(type!=='tv'||!$('cf312AutoNext')?.checked||(totalEpisodes&&episode>=totalEpisodes))return;
    cancelAutoNext();let count=10;const o=$('cf312NextOverlay');if(!o)return;o.hidden=false;o.setAttribute('aria-hidden','false');o.style.display='grid';$('cf312NextCount').textContent=count;
    autoNextCountdown=setInterval(()=>{count--;$('cf312NextCount').textContent=count;if(count<=0){cancelAutoNext();selectEpisode(episode+1,{showAd:false});}},1000);
  }
  function scheduleAutoNext(){cancelAutoNext();if(type!=='tv'||!$('cf312AutoNext')?.checked)return;const mins=Math.max(8,Number(episodeRuntime)||42);autoNextTimer=setTimeout(showAutoNext,Math.max(60_000,(mins*60_000)-25_000));}
  ...
  frame.addEventListener('load',()=>{
    ...
    const latency=Math.max(1,Math.round(performance.now()-serverStartedAt));serverLatency.set(server.id,latency);saveLatency();fallbackAttempts=0;
    renderServers();
    frameWrap.classList.remove('is-loading');
    $('serverNotice').hidden=true;
    setPlayerStatus('ready','','');updateHealth('online',serverLatency.get(server.id));showToolbar();
    saveLastWatch();scheduleAutoNext();
  });
  $('fastestServerBtn').onclick=useFastestServer;
  ...
  const cancelNextBtn=$('cf312CancelNext'), playNextBtn=$('cf312PlayNext');
  const closeNextOverlay=(event)=>{if(event){event.preventDefault();event.stopPropagation();}cancelAutoNext();};
  const playNextEpisode=(event)=>{if(event){event.preventDefault();event.stopPropagation();}const target=episode+1;cancelAutoNext();setTimeout(()=>selectEpisode(target,{showAd:false}),0);};
  cancelNextBtn.addEventListener('click',closeNextOverlay);
  playNextBtn.addEventListener('click',playNextEpisode);
  playNextBtn.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){playNextEpisode(event)}});
  $('cf312AutoNext').checked=localStorage.getItem('cineflex_auto_next')!=='off';
  $('cf312AutoNext').onchange=e=>{localStorage.setItem('cineflex_auto_next',e.target.checked?'on':'off');e.target.checked?scheduleAutoNext():cancelAutoNext()};
