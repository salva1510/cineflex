(() => {
  ...
  function renderDaily(item){const box=document.getElementById('cf-daily-pick');if(!box||!item)return;box.style.backgroundImage=`url(${IMG_URL}${backdrop(item)})`;box.innerHTML=`<div class="cf-daily-content"><span>CineFlex Daily Pick</span><h2>${esc(title(item))}</h2><p>${esc((item.overview||'A fresh pick selected from your recent CineFlex activity.').slice(0,180))}</p><button type="button"><i class="fa-solid fa-play"></i> View Pick</button></div>`;box.querySelector('button').onclick=()=>showDetails(item)}
