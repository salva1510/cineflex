...
async function init() {
  try {
    // Idinagdag ang netflixMovies endpoint (with_companies=213 ay para sa Netflix)
    const [trd, marvel, anime, fil, kd, kp, kids, pinoyAction, dramabox, netflixMovies] = await Promise.all([
      ...
      fetch(`${BASE_URL}/discover/movie?api_key=${API_KEY}&with_companies=213&sort_by=popularity.desc`).then(r => r.json())
    ]);
    ...
    // Render para sa Netflix Movies at DramaBox
...
async function viewAll(containerId) {
    ...
    if (containerId === "trending-today") {
        ...
        // Ginamit ang with_companies=213 para sa Netflix Movies View All
        url = `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_companies=213&sort_by=popularity.desc`;
