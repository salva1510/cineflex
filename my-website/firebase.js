// ===========================================
// CINEFLEX FIREBASE ENGINE v9 — BUILD 300 CORE
// ===========================================
...
// Single-app boot guard for every CineFlex page.
if (!window.__CINEFLEX_FIREBASE_APP__) {
    window.__CINEFLEX_FIREBASE_APP__ = firebase.apps.length
        ? firebase.app()
        : firebase.initializeApp(firebaseConfig);
}
const cineflexFirebaseApp = window.__CINEFLEX_FIREBASE_APP__;

// Firebase Services
const auth = cineflexFirebaseApp.auth();
const db = cineflexFirebaseApp.firestore();
...
window.cineflexAuthReadyPromise = window.cineflexAuthReadyPromise || new Promise(resolve => { window.__resolveCineflexAuthReady = resolve; });
...
async function bootCineFlexAuth() {
    if (window.__CINEFLEX_AUTH_BOOTED__) return window.cineflexAuthReadyPromise;
    window.__CINEFLEX_AUTH_BOOTED__ = true;
    window.cineflexAuthReady = false;
    window.cineflexRedirectChecked = false;
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        if (window.__resolveCineflexAuthReady) { window.__resolveCineflexAuthReady(user); window.__resolveCineflexAuthReady = null; }
        window.dispatchEvent(new CustomEvent("cineflex-auth-state", { detail: { user } }));
