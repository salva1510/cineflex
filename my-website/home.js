...
async function init() {
  try {
    // Ginamit ang /search/tv para sigurado na may poster_path na kasama ang Cocomelon data
    const [trd, marvel, anime, fil, kd, kp, kids, pinoyAction, dramabox, netflixMovies, cocomelonData] = await Promise.all([
      ...
      fetch(`${BASE_URL}/search/tv?api_key=${API_KEY}&query=cocomelon`).then(r => r.json())
    ]);
    ...
    // I-render ang bagong Cocomelon scroller gamit ang nirefresh na data
...
async function viewAll(containerId) {
    ...
    if (containerId === "trending-today") {
        ...
        url = `${BASE_URL}/search/tv?api_key=${API_KEY}&query=cocomelon`;
