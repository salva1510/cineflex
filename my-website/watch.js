(() => {
  ...
  function triggerFreeAd(){
    // Build 300.6: second ad trigger point — a newly selected episode.
    // Never guess while Firebase membership is unresolved; skipping protects VIP users.
    if(!membershipResolved||isVip)return false;
    const now=Date.now();
    ...
    s.id='cineflex-watch-play-popad';
    s.src=`https://bashsecret.com/03/53/7d/03537deb3b1a6012bf51de011865aed1.js?cf_episode=${now}`;
    s.async=true;
    s.onload=s.onerror=()=>setTimeout(()=>s.remove(),15000);
