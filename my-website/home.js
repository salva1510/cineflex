...
// MAIN SHOW DETAILS FUNCTION (In-Modal Support)
async function showDetails(item) {
  ...
  closeSearch(); // Close search if opening from search results
  ...
  // Mini Recommendations (More Like This)
  ...
  recContainer.innerHTML = recs.results.slice(0, 8).map(r => `
    <div class="rec-item" onclick='showDetails(${JSON.stringify(r).replace(/'/g, "&apos;")})'>
        ...
        <div class="rec-info">
            ...
            <p>${r.overview || 'Suggested for you.'}</p>
        </div>
    </div>
  `).join('');
...
const processSearch = async (q) => {
    ...
    try {
        ...
        resultsDiv.innerHTML = res.results.filter(i => i.poster_path || i.backdrop_path).map(item => `
...
async function loadEpisodes(seriesId, seasonNum) {
    ...
    document.getElementById("episode-list").innerHTML = data.episodes.map(e => {
        const thumb = e.still_path ? `https://image.tmdb.org/t/p/w300${e.still_path}` : 'https://via.placeholder.com/300x170?text=No+Preview';
        return `
