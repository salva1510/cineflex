(() => {
  ...
  const activeLayer = () => {
    // Only treat a layer as active when it is explicitly opened. Off-canvas drawers
    // still have width/height while closed, which previously trapped ArrowDown.
    const selectors = [
      '.modal.active', '.modal.show', '.modal.open',
      '.search-overlay.active', '.search-overlay.show', '.search-overlay.open',
      '.menu-drawer.active', '.menu-drawer.open', '.menu-drawer.show',
      '[role="dialog"][aria-hidden="false"]'
    ].join(',');
    const layers = [...document.querySelectorAll(selectors)].filter(isVisible);
  ...
  function firstPoster() {
    // Prefer the first real card in the first visible homepage scroller below the hero.
    const rows = [...document.querySelectorAll('.scroller,.cf-top10-scroller,.main-list,.tv-list')].filter(isVisible);
    for (const row of rows) {
      const card = [...row.querySelectorAll(POSTER_SELECTOR)].find(isVisible);
      if (card && !card.closest('#banner,.cf-hero-video')) return card;
    }
  ...
  function handleKey(event) {
    ...
    if (direction) {
      ...
      if (event.key === 'ArrowDown' && (!focused || focused.closest?.('#banner') || focused === bannerPlay())) {
        const target = firstPoster();
        if (target && focusElement(target)) return;
        // Content may still be rendering; scroll once and retry after the next paint.
        window.scrollBy({ top: Math.max(260, innerHeight * 0.55), behavior: 'smooth' });
        setTimeout(() => focusElement(firstPoster()), 180);
        return;
      }
  ...
  function init() {
    ...
    if (!window.CF_TV_PRO_ENGINE) window.addEventListener('keydown', handleKey, { capture: true });
