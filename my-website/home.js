...
function playSpecificEpisode(epNum, element) {
    ...
    // Ina-activate ang pop-under ad pag-play ng episode
    triggerPopUnder();
}

// --- UPDATED PEACHIFY MOVIE LINK ---
function startPlayback() {
    ...
    // Ina-activate ang pop-under ad pag-play ng movie
    triggerPopUnder();
}
...
function displayCards(data, containerId) {
  ...
  // Magdagdag ng "View All" card sa dulo at ipasa ang mismong pangalan ng section id
...
// --- UPDATED VIEW ALL: KUKUHA NG LAHAT NG MOVIES PARA SA BAWAT SECTION ---
async function viewAll(containerId) {
    const resultsDiv = document.getElementById("search-results");
    const overlay = document.getElementById("search-overlay");
    if (!resultsDiv || !overlay) return;

    // Maglagay ng Loading text habang kinukuha ang buong listahan
    resultsDiv.innerHTML = "<div style='color:white; text-align:center; width:100%; padding:20px;'>Loading full list...</div>";
    overlay.style.display = "block";

    let url = "";
    
    // Alamin kung anong section ang pinindot para itugma sa tamang TMDB API endpoint
    if (containerId === "trending-today") {
        ...
        // Para sa continue watching, gamitin lang kung ano ang nasa local storage natin
    ...
    try {
        // Mag-fetch ng magkasabay na page 1 at page 2 para maging MARAMI at BUKOD-TANGI ang listahan ng movies
        const [page1, page2] = await Promise.all([
            fetch(`${url}&page=1`).then(r => r.json()),
            fetch(`${url}&page=2`).then(r => r.json())
        ]);
        
        // Pagsamahin ang dalawang pahina para lumabas lahat ng movie details
        const allItems = [...(page1.results || []), ...(page2.results || [])];

        // I-render na sa grid layout ang kumpletong mga pelikula ng section na pinili
