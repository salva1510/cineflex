      ...
      try {
        ...
        player = new YT.Player(mount.id, {
          ...
          events: {
            onReady(event) {
              if (token !== trailerToken) return;
              try { event.target.mute(); event.target.setVolume(0); event.target.playVideo(); const frame=event.target.getIframe?.(); if(frame){frame.setAttribute('allow','autoplay; encrypted-media; fullscreen; picture-in-picture'); frame.setAttribute('playsinline','');} } catch (_) {}
            },
            onStateChange(event) {
              if (token !== trailerToken) return;
