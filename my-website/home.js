...
async function init() {
    ...
    // Config para sa lahat ng rows
    const sections = [
        { path: 'trending/all/day', id: 'trending-today' },
        { path: 'discover/tv?with_original_language=ko&with_genres=18', id: 'kdrama-list' }, // K-DRAMA
    ...
    for (const sec of sections) {
        try {
            const response = await fetch(`${BASE_URL}/${sec.path}&api_key=${API_KEY}`);
            const data = await response.json();
            if (sec.id === 'trending-today') {
                trendingItems = data.results;
                setBanner(data.results[0]);
            }
            displayCards(data.results, sec.id);
        } catch (e) { console.error("Error loading " + sec.id, e); }
    }
    updateContinueUI();
}
...
async function showDetails(item) {
    ...
    document.getElementById("modal-cast").innerHTML = credits.cast.slice(0, 10).map(c => `
        <div class="cast-card">
            ...
            <p style="font-size:0.7rem; color:white;">${c.name}</p>
        </div>`).join('');
...
// (Setup Gestures function should be here as defined in previous messages)
function setupGestures() { /* Same as previous version */ }
async function enterCinemaMode() { /* Same as previous version */ }
