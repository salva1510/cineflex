...
Mawawala ang profile data nito sa account mo.`);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    ensureDrawerProfiles();
    ensureModal();
    const applyProfileAuthState = (user) => { updateDrawerAccount(user); if (user) loadProfiles(); };
    window.addEventListener("cineflex-auth-state", event => applyProfileAuthState(event.detail && event.detail.user));
    window.addEventListener("cineflex-logout", () => applyProfileAuthState(null));
    if (window.cineflexAuthReady) applyProfileAuthState(window.currentUser || null);
    else loadProfiles();
  });
