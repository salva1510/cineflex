...
async function enterCinemaMode() {
  ...
  try {
    ...
    // IPAD AIR 2 FIX: Mas ligtas na pag-check sa screen orientation API para hindi mag-crash sa iOS Safari
...
// Dagdag na listener para sa lumang WebKit version ng fullscreen (iOS Safari)
...
async function playTrailer() {
    ...
    try {
        ...
        if (trailerItem && trailerItem.key) {
            ...
            // IPAD AIR 2 FIX: I-delay nang bahagya ang ads para hayaang makarga ang youtube video nang walang pop-up restriction blockage
...
function switchServer(serverNum) {
    ...
    updateServerTabsUI();
    updateVideoSource();
    setTimeout(triggerPopUnder, 600);
}
...
function playSpecificEpisode(epNum, element) {
    ...
    // IPAD AIR 2 FIX: Ginamitan ng setTimeout para hindi maharang ng Safari ang click action habang nagpe-play ang player
...
function startPlayback() {
    ...
    // IPAD AIR 2 FIX: Buffer delay sa ad popunder para iwas player block sa iOS
    setTimeout(triggerPopUnder, 1000);
}
