...
// --- BUILD 300.6: VIP AD-FREE / FREE TWO-POINT POP-AD SYSTEM ---
// Trigger point 1: Play Movie / open a new title.
// Trigger point 2: Select a new episode from the watch page.
// Safety rule: when membership is still unresolved, skip the ad rather than risk showing one to VIP.
const CINEFLEX_PLAY_AD_COOLDOWN_MS = 1500;
let cineflexLastPlayAdAt = 0;
let cineflexPlayAdSequence = 0;

function cineflexMembershipState() {
  const api = window.CineFlexMembership;
  if (!api || typeof api.getState !== "function") {
    return { resolved: !window.auth?.currentUser, vip: false };
  }
  const state = api.getState() || {};
  return {
    resolved: Boolean(state.loaded) || !window.auth?.currentUser,
    vip: Boolean(state.vip || api.isVip?.())
  };
}

function triggerPlaybackPopAd() {
  try {
    if (window.CineFlexPlugins && !window.CineFlexPlugins.isEnabled('ads')) return false;
    const membership = cineflexMembershipState();
    if (!membership.resolved || membership.vip) return false;
    ...
    console.log("CineFlex pop-ad blocked or failed:", error);
    return false;
  }
}

// Compatibility wrappers retained for older buttons.
