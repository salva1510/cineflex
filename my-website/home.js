...
async function showDetails(item) {
  ...
  try {
    ...
    // --- 2-COLUMN GRID SYSTEM STYLE PARA SA RECS/MORE LIKE THIS ---
...
// --- BAGONG FUNCTION PARA SA TRAILER SYSTEM ---
async function playTrailer() {
    if (!currentItem) return;
    
    const type = currentTVState.type; // 'movie' o 'tv'
    const id = currentItem.id;
    const iframe = document.getElementById("modal-video-iframe");
    const playerContainer = document.getElementById("modal-player-container");

    try {
        // I-fetch ang list ng videos mula sa TMDB API
        const res = await fetch(`${BASE_URL}/${type}/${id}/videos?api_key=${API_KEY}`).then(r => r.json());
        
        // Maghanap ng YouTube clip na ang type ay "Trailer", o fallback sa "Teaser" kung walang main trailer
        ...
        if (trailerItem && trailerItem.key) {
            ...
            // I-embed ang YouTube link na may autoplay enabled
            if (iframe) iframe.src = `https://www.youtube.com/embed/${trailerItem.key}?autoplay=1&rel=0&modestbranding=1`;
            
            // Awtomatikong mag-scroll pataas ang modal para agad makita ang player area
...
// --- 2-COLUMN GRID SYSTEM STYLE PARA SA EPISODES LIST ---
...
// --- OFFICIAL NETFLIX TOP 10 CARD RENDERER (NUMERO SA HARAP) ---
