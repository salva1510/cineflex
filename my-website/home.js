...
const SERVER_2_URL = "https://1embed.cc";

let currentServer = 1; // Default to Server 1 (Peachify)
...
async function showDetails(item) {
  currentItem = item;
  currentServer = 1; // Always reset to Server 1 when open a new item modal
  const type = (item.first_air_date || item.name || item.media_type === 'tv') ? 'tv' : 'movie';
  const modal = document.getElementById("details-modal");
  const playerContainer = document.getElementById("modal-player-container");
  const serverContainer = document.getElementById("modal-server-container");
  ...
  try {
    ...
    // Build Server Options inside the UI
    renderServerSelection(type);
...
function renderServerSelection(type) {
  const container = document.getElementById("modal-server-container");
  if (!container) return;
  
  container.innerHTML = `
    <button class="srv-btn ${currentServer === 1 ? 'active' : ''}" onclick="switchServer(1, '${type}')">Server 1 (Peachify)</button>
    <button class="srv-btn ${currentServer === 2 ? 'active' : ''}" onclick="switchServer(2, '${type}')">Server 2 (1Embed)</button>
  `;
}

function switchServer(serverNum, type) {
  if (currentServer === serverNum) return;
  currentServer = serverNum;
  renderServerSelection(type);
  
  // If player is already open and running, refresh frame immediately with new host source
  const playerContainer = document.getElementById("modal-player-container");
  if (playerContainer && playerContainer.style.display === "block") {
     if (type === 'tv') {
       updatePlayerSource(type, currentTVState.episode);
     } else {
...
function updatePlayerSource(type, epNum = 1) {
  const iframe = document.getElementById("modal-video-iframe");
  if (!iframe || !currentItem) return;

  if (type === 'movie') {
    iframe.src = currentServer === 1 
      ? `${SERVER_1_URL}/embed/movie/${currentItem.id}`
      : `${SERVER_2_URL}/embed/movie/${currentItem.id}`;
  } else {
    iframe.src = currentServer === 1
      ? `${SERVER_1_URL}/embed/tv/${currentItem.id}/${currentTVState.season}/${epNum}`
      : `${SERVER_2_URL}/embed/tv/${currentItem.id}/${currentTVState.season}/${epNum}`;
  }
}
...
function closeModal() { 
  ...
  document.getElementById("modal-server-container").style.display = "none"; 
