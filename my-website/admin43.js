(() => {
  ...
  const OWNER_UID='M3GxGOkF9ZWoezkICNpvS5ID1Hz2';
...
<section id="audit" class="page"><div class="panel"><div class="manager-head"><div><h2>Admin Audit Trail</h2><p class="muted">Recent security and moderation actions.</p></div><button id="cf43RefreshLogs" class="ghost-btn">Refresh</button></div><div id="cf43Logs">Loading…</div></div></section>`);
  ...
  async function resolveRole(user){
    if(user.uid===OWNER_UID || String(user.email||'').trim().toLowerCase()===OWNER_EMAIL)return 'super_admin';
    if(window.CINEFLEX_ADMIN_ROLE==='super_admin')return 'super_admin';
  ...
  function vipExpiryText(data){const plan=String(data?.plan||(data?.vip?'vip':'free')).toLowerCase();if(!data||data.active===false||plan!=='vip')return 'FREE';if(data.lifetime===true)return 'VIP · Lifetime';const ms=Number(data.expiresAtMs)||dateVal(data.vipExpiry||data.expiresAt||data.vipUntil);if(ms&&ms<=Date.now())return 'EXPIRED';return ms?'VIP until '+new Date(ms).toLocaleString():'VIP';}
  async function loadVipStatus(){
    const uid=$('cf43VipUid').value.trim(); if(!uid)return $('cf43VipResult').textContent='Select a user to view membership status.';
    try{const snap=await db.collection('users').doc(uid).collection('membership').doc('status').get();const data=snap.exists?snap.data():{};const u=users.find(x=>x.uid===uid)||{};$('cf43VipResult').innerHTML=`<b>${esc(u.email||u.displayName||uid)}</b><br>Current plan: <strong>${esc(vipExpiryText(data))}</strong>`;}catch(e){$('cf43VipResult').textContent='Unable to read VIP status: '+e.message}
  }
  async function setVip(active){
    if(!can('super_admin'))return toast43('Super admin access required');
    const uid=$('cf43VipUid').value.trim(); if(!uid)return toast43('Enter or select a user UID');
    const ref=db.collection('users').doc(uid).collection('membership').doc('status');
    try{
      if(active){const choice=$('cf43VipDuration').value;const lifetime=choice==='lifetime';const expiresAtMs=lifetime?4102444800000:Date.now()+Number(choice)*86400000;await ref.set({active:true,vip:true,plan:'vip',lifetime,expiresAtMs,vipExpiry:firebase.firestore.Timestamp.fromMillis(expiresAtMs),expiresAt:firebase.firestore.Timestamp.fromMillis(expiresAtMs),updatedAt:ts(),updatedBy:me.email,updatedByUid:me.uid},{merge:true});await log('vip_upgraded',uid,{duration:choice,expiresAtMs,lifetime});toast43('VIP access activated');}
      else{await ref.set({active:false,vip:false,plan:'free',lifetime:false,expiresAtMs:0,vipExpiry:null,expiresAt:null,updatedAt:ts(),updatedBy:me.email,updatedByUid:me.uid},{merge:true});await log('vip_removed',uid);toast43('VIP access removed');}
