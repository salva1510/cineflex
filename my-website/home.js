...
async function init() {
  try {
    // UPDATED: Kasama na ang DramaBox API request gamit ang sikat na micro-drama short formats sa TV (Soap/Mini-series keywords o alternative short networks)
    const [trd, marvel, anime, fil, kd, kp, kids, pinoyAction, dramabox] = await Promise.all([
      ...
      fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&with_keywords=210024|234151&sort_by=popularity.desc`).then(r => r.json()) // DramaBox style keywords (micro-series/short drama)
    ]);
    ...
    // NATIVE INJECTION: Render ng DramaBox style list gamit ang portrait format function
  ...
  try {
    ...
    if (recContainer) {
      recContainer.innerHTML = recs.results.slice(0, 8).map(r => `
        <div class="rec-item" tabindex="0" onclick='showDetails(${JSON.stringify(r).replace(/'/g, "&apos;")})'>
...
function displayCards(data, containerId) {
  ...
  container.innerHTML = data.filter(i => i.poster_path).map(item => `
    <div class="card" tabindex="0" onclick='showDetails(${JSON.stringify(item).replace(/'/g, "&apos;")})'>
...
// --- NEW SPECIFIC FUNCTION PARA SA DRAMABOX PORTRAIT LAYOUT ---
function displayDramaBoxCards(data, containerId) {
  const container = document.getElementById(containerId);
  if(!container || !data) return;
  
  container.innerHTML = data.filter(i => i.poster_path).map((item, idx) => {
    // Nag-ge-generate ng random episode range mula 60-100 para magmukhang DramaBox series talaga ang UI display
...
// --- BUO AT MAAYOS NA VIEW ALL FUNCTION ---
async function viewAll(containerId) {
    ...
    resultsDiv.innerHTML = "<div style='color:white; text-align:center; width:100%; padding:20px;'>Loading list...</div>";
    ...
    } else if (containerId === "dramabox-list") { // ADDED: Endpoint structure integration para sa viewAll list configuration
        url = `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_keywords=210024|234151&sort_by=popularity.desc`;
    ...
    try {
        ...
        resultsDiv.innerHTML = "<div style='color:white; text-align:center; width:100%; padding:20px;'>Failed to load items.</div>";
    }
}

// --- SYSTEMA NG AUTOMATIC SCROLL LOADING ---
...
// --- NETFLIX-STYLE REMOTE NAVIGATION SYSTEM ---
let selectedIndex = 0;
const getFocusables = () => document.querySelectorAll('button, [onclick], .card, .episode-item, .nav-item, .search-card');
