...
async function showDetails(item) {
  ...
  try {
    ...
    // STRUCTURE NG MORE LIKE THIS (PARA SA PINALAKING THUMBNAIL CONTAINER)
      ...
      recContainer.innerHTML = recs.results.slice(0, 8).map(r => `
        <div class="rec-item" tabindex="0" onclick='showDetails(${JSON.stringify(r).replace(/'/g, "&apos;")})'>
            <div class="rec-thumb-container">
                <img src="${IMG_URL}${r.backdrop_path || r.poster_path}" class="rec-thumb">
            </div>
            <div class="rec-info">
                <h4>${r.title || r.name}</h4>
                <p>${r.overview || 'Watch now.'}</p>
            </div>
        </div>`).join('');
    }
...
// STRUCTURE NG EPISODE ITEM (PARA SA PINALAKING THUMBNAIL CONTAINER)
      ...
      epList.innerHTML = data.episodes.map(e => `
            <div class="episode-item" tabindex="0" onclick="playSpecificEpisode(${e.episode_number}, this)">
                <div class="ep-thumb-container">
                    <img src="${e.still_path ? IMG_URL + e.still_path : 'https://via.placeholder.com/300x170'}" class="ep-thumb">
                </div>
                <div class="ep-info">
                    <h4>${e.episode_number}. ${e.name}</h4>
                    <p>${e.overview || 'Watch now.'}</p>
                </div>
            </div>`).join('');
    }
}
...
// --- OFFICIAL NETFLIX TOP 10 CARD RENDERER (NUMERO SA HARAP NG POSTER) ---
...
let selectedIndex = 0;
const getFocusables = () => document.querySelectorAll('button, [onclick], .card, .episode-item, .nav-item, .search-card, .netflix-item-container');
