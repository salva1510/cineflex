...
function setupGestures() {
    ...
    player.addEventListener('touchend', () => { setTimeout(() => { indicator.style.display = 'none'; }, 500); });
}
...
async function init() {
    ...
    for (const item of config) {
        ...
        if (item.id === 'trending-today') { trendingItems = data.results; setBanner(data.results[0]); }
...
async function showDetails(item) {
    ...
    if(isTV) {
        const d = await fetch(`${BASE_URL}/tv/${item.id}?api_key=${API_KEY}`).then(r => r.json());
        setupSeasonSelector(d);
    }

    const rec = await fetch(`${BASE_URL}/${type}/${item.id}/recommendations?api_key=${API_KEY}`).then(r => r.json());
...
async function loadEpisodes(id, sn) {
    ...
    document.getElementById("episode-list").innerHTML = d.episodes.map(e => `
        <div class="episode-item" onclick="playSpecificEpisode(${e.episode_number})" style="display:flex; gap:10px; margin-bottom:10px; background:#111; padding:10px; border-radius:8px; cursor:pointer;">
            <img src="${IMG_URL}${e.still_path}" style="width:120px; border-radius:5px;">
