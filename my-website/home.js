...
const SERVER_2_URL = "https://cinebit-api.vercel.app";
...
async function enterCinemaMode() {
  ...
  } catch (err) { console.log("Cinema mode error:", err); }
}
...
async function init() {
  try {
    const [trd, marvel, anime, fil, kd, kp, kids, pAction] = await Promise.all([
      ...
      fetch(`${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=28&with_original_language=tl`).then(r => r.json())
    ]);
    ...
    displayCards(pAction.results, "pinoy-action-list");
...
function setupSeasonSelector(series) {
    ...
    seasonSelect.innerHTML = series.seasons.filter(s => s.season_number > 0).map(s => `<option value="${s.season_number}">Season ${s.season_number}</option>`).join('');
...
async function loadEpisodes(seriesId, seasonNum) {
    ...
    document.getElementById("episode-list").innerHTML = data.episodes.map(e => `
        <div class="episode-item" onclick="playSpecificEpisode(${e.episode_number}, this)">
            ...
            <div class="ep-info"><h4>${e.episode_number}. ${e.name}</h4></div>
        </div>`).join('');
}

function updateVideoSource() {
    const iframe = document.getElementById("modal-video-iframe");
    const baseUrl = activeServer === 1 ? SERVER_1_URL : SERVER_2_URL;
    if (currentTVState.type === 'tv') {
        iframe.src = `${baseUrl}/embed/tv/${currentItem.id}/${currentTVState.season}/${currentTVState.currentEpNum}`;
    } else {
        iframe.src = `${baseUrl}/embed/movie/${currentItem.id}`;
    }
}

function switchServer(serverNum) { activeServer = serverNum; updateServerTabsUI(); updateVideoSource(); triggerPopUnder(); }
function updateServerTabsUI() {
    document.getElementById("srv1-btn").className = activeServer === 1 ? "server-btn active" : "server-btn";
    document.getElementById("srv2-btn").className = activeServer === 2 ? "server-btn active" : "server-btn";
}
...
async function viewAll(containerId) {
    ...
    const mapping = {
        ...
        "pinoy-action-list": `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=28&with_original_language=tl`
    };
    
    url = mapping[containerId];
    try {
        const res = await fetch(`${url}&page=1`).then(r => r.json());
    ...
    } catch (err) { resultsDiv.innerHTML = "Error loading."; }
}
