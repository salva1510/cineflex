...
async function init() {
  try {
    // UPDATED: May 'without_genres=16' para harangan ang Anime at siguraduhing pure live-action short dramas lang ang lalabas
    const [trd, marvel, anime, fil, kd, kp, kids, pinoyAction, dramabox] = await Promise.all([
      ...
      fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&with_genres=18&without_genres=16&with_keywords=210024|234151&sort_by=popularity.desc`).then(r => r.json())
    ]);
    ...
    // Render ng DramaBox gamit ang vertical layout function natin
...
// --- RENDERING FUNCTION PARA SA DRAMABOX (VERTICAL CARDS) ---
function displayDramaBoxCards(data, containerId) {
  const container = document.getElementById(containerId);
  if(!container || !data) return;
  
  container.innerHTML = data.filter(i => i.poster_path).map((item) => {
    // Kunwari ay may 60-100 episodes ang isang short drama para sa aesthetic ng app
...
async function viewAll(containerId) {
    ...
    if (containerId === "trending-today") {
        ...
        // UPDATED: 'without_genres=16' para manatiling live-action pati ang listahan sa View All page
        url = `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_genres=18&without_genres=16&with_keywords=210024|234151&sort_by=popularity.desc`;
