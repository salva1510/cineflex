...
async function init() {
  try {
    const [trd, anime, fil, kd, kp, kids] = await Promise.all([
      ...
      fetch(`${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=16,10751`).then(r => r.json()) // Fetch for Kids & Family
    ]);
    ...
    displayCards(kids.results, "kids-list"); // Display Kids Section
