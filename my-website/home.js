...
function showSkeleton(containerId) {
  ...
  if (container) container.innerHTML = Array(6).fill('<div class="skeleton"></div>').join('');
}
...
function setBanner(item) {
  ...
  setTimeout(() => {
    ...
    if(bannerDesc) bannerDesc.textContent = item.overview ? item.overview.substring(0, 160) + "..." : "";
...
async function showDetails(item) {
  ...
  modal.style.display = "block"; // Changed to block for proper scrolling
...
async function loadEpisodes() {
  ...
  // Auto-scroll modal back to top when season changes
...
async function initGenreBrowse() {
  ...
  select.addEventListener("change", async () => {
    ...
    if(data) displayList(data.results.map(i => ({...i, media_type: 'movie'})), "genre-movies-list");
  });
}
...
function saveContinueWatching(item, season = null, episode = null) {
  ...
  list.unshift({ id: item.id, poster: item.poster_path, media_type: item.media_type || "movie", season, episode });
...
function renderContinueWatching() {
  ...
  let list = JSON.parse(localStorage.getItem("continueWatching")) || [];
  if (!row || list.length === 0) { if(row) row.style.display = "none"; return; }
...
async function resumeWatching(savedItem) {
  const url = savedItem.media_type === "tv" ? `${BASE_URL}/tv/${savedItem.id}?api_key=${API_KEY}` : `${BASE_URL}/movie/${savedItem.id}?api_key=${API_KEY}`;
