(() => {
  ...
  function cancelAutoNext(){clearTimeout(autoNextTimer);clearInterval(autoNextCountdown);const o=$('cf312NextOverlay');if(o)o.hidden=true;}
  function showAutoNext(){
    if(type!=='tv'||!$('cf312AutoNext')?.checked||(totalEpisodes&&episode>=totalEpisodes))return;
    cancelAutoNext();let count=10;const o=$('cf312NextOverlay');if(!o)return;o.hidden=false;$('cf312NextCount').textContent=count;
  ...
  $('cf312CancelNext').onclick=cancelAutoNext;
  $('cf312PlayNext').onclick=()=>{cancelAutoNext();selectEpisode(episode+1,{showAd:false})};
