...
(() => {
  ...
  function setupAutoHide(){
    ...
    if(!box||box.dataset.cfAutoHide==='1')return;
    box.dataset.cfAutoHide='1';
    let hideTimer=null;
    const hide=()=>{
      if(box.querySelector('.cf61-server-tabs.is-open'))return;
      box.classList.remove('cf-controls-visible');
    };
    const show=(delay=3200)=>{
      box.classList.add('cf-controls-visible');
      clearTimeout(hideTimer);
      hideTimer=setTimeout(hide,delay);
    };
    const hotspot=document.createElement('div');
    hotspot.className='cf-player-controls-hotspot';
    hotspot.setAttribute('aria-label','Show player controls');
    hotspot.addEventListener('pointerdown',e=>{e.preventDefault();show(4200);});
    box.appendChild(hotspot);
    box.addEventListener('mousemove',()=>show(2600),{passive:true});
    box.addEventListener('mouseenter',()=>show(2600),{passive:true});
    box.addEventListener('keydown',()=>show(4200));
    box.addEventListener('focusin',()=>show(4200));
    box.addEventListener('cf61:panel-open',()=>show(120000));
    box.addEventListener('cf61:panel-close',()=>show(2600));
