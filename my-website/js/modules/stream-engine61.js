...
(() => {
  ...
  function renderTabs() {
    ...
    if (!document.getElementById('cf61-server-toggle')) {
      ...
      toggle.addEventListener('click', () => {
        ...
        player.dispatchEvent(new CustomEvent(panelOpen ? 'cf61:panel-open' : 'cf61:panel-close'));
        window.cf60ShowControls?.(panelOpen ? 120000 : 2600);
      });
  ...
  function load(reason = 'manual') {
    ...
    document.getElementById('modal-player-container')?.dispatchEvent(new CustomEvent('cf61:panel-close'));
    window.cf60ShowControls?.(2200);
