(() => {
  ...
  const placementControl = $('adPlacementMode');
  const controls = {
  ...
  function apply(data={}){
    ...
    if (placementControl) placementControl.value = ['play_episode','play_only','first_session'].includes(data.adPlacement) ? data.adPlacement : 'play_episode';
  ...
  async function publish(){
    ...
    try {
      ...
      await REF().set({plugins,adPlacement:placementControl?.value||'play_episode',updatedAt:firebase.firestore.FieldValue.serverTimestamp(),updatedBy:(auth.currentUser&&(auth.currentUser.email||auth.currentUser.uid))||'admin'},{merge:true});
