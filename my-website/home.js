...
// --- UPDATED DISPLAY CARDS WITH GIANT NETFLIX DESIGN 1 NUMBERS ---
function displayCards(data, containerId) {
  ...
  container.innerHTML = data.filter(i => i.poster_path).map((item, index) => {
    let trendingBadge = "";
    let isNetflix = (containerId === "netflix-movies-list" && index < 10);
    
    if (isNetflix) {
        trendingBadge = `<div class="netflix-big-number">${index + 1}</div>`;
    }

    return `
    <div class="${isNetflix ? 'card netflix-card-wrapper' : 'card'}" tabindex="0" onclick='showDetails(${JSON.stringify(item).replace(/'/g, "&apos;")})'>
        ${trendingBadge}
        <img src="${IMG_URL}${item.poster_path}" loading="lazy" class="${isNetflix ? 'netflix-poster' : ''}">
    </div>`;
  }).join('');

  const viewAllCard = document.createElement('div');
  viewAllCard.className = containerId === "netflix-movies-list" ? "card netflix-card-wrapper view-all-card" : "card view-all-card";
