...
window.cfOpenMusic=window.cfOpenMusicHub=function(){
  if(window.CineFlexPlugins && !window.CineFlexPlugins.isEnabled('youtubeMusic')){window.showToast?.('YT Music is currently disabled.');return false;}
  try{closeMenuDrawer()}catch(e){}open();
};
...
window.CineFlexPlugins?.register('youtubeMusic',{open:window.cfOpenMusic,close});
