(function () {
  ...
  const AD_URL = 'js/core/cineflex-pop-provider3056.js?v=305.6';
  const COOLDOWN_MS = 1200;
  const PAGE = (location.pathname.split('/').pop() || 'index.html').toLowerCase();
  const IS_HOMEPAGE = PAGE === '' || PAGE === 'index.html' || PAGE === 'home.html';
  const SECOND_PAGE_ACTIONS = new Set(['category-open', 'load-more']);
  const ALLOWED_ACTIONS = new Set(['play-now', 'episode-click', ...SECOND_PAGE_ACTIONS]);
  const LOG_KEY = 'cineflex_ad_events_301';
  const SESSION_KEY = 'cineflex_ad_first_play_3012';
  const CATEGORY_SESSION_KEY = `cineflex_category_ad_${PAGE}_3055`;
  ...
  function placementAllows(action) {
    if (SECOND_PAGE_ACTIONS.has(action)) {
      return sessionStorage.getItem(CATEGORY_SESSION_KEY) !== '1';
    }
  ...
  function record(action, result, reason) {
    const event = { action, result, reason, placement, at: Date.now(), build: '305.6' };
    try {
  ...
  function trigger(kind) {
    ...
    try {
      ...
      if (SECOND_PAGE_ACTIONS.has(action)) sessionStorage.setItem(CATEGORY_SESSION_KEY, '1');
