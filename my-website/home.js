...
function downloadMedia() {
    ...
    if (currentTVState.type === 'tv') {
        downloadUrl = `${SERVER_1_URL}/download/tv/${currentItem.id}/${currentTVState.season}/${currentTVState.currentEpNum}`;
    } else {
        downloadUrl = `${SERVER_1_URL}/download/movie/${currentItem.id}`;
    }
...
function updateServerTabsUI() { /* Implementation omitted for brevity but kept consistent */ }
...
function closeModal() { document.getElementById("details-modal").style.display = "none"; document.getElementById("modal-video-iframe").src = ""; document.body.style.overflow = "auto"; if (screen.orientation && screen.orientation.unlock) screen.orientation.unlock(); }
...
function addToContinueWatching(item) { continueWatching = continueWatching.filter(i => i.id !== item.id); continueWatching.unshift(item); if (continueWatching.length > 10) continueWatching.pop(); localStorage.setItem("cineflex_recent", JSON.stringify(continueWatching)); updateContinueUI(); }
function updateContinueUI() { const section = document.getElementById("continue-watching-section"); if(continueWatching.length > 0 && section) { section.style.display = "block"; displayCards(continueWatching, "continue-list"); } }
async function viewAll(containerId) { /* Logic remains same */ }
init();
