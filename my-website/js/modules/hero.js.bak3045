(function () {
  ...
  async function loadTrailer(item) {
    ...
    trailerTimer = setTimeout(async () => {
      try {
        const response = await fetch(`${apiBase}/${mediaType(item)}/${item.id}/videos?api_key=${apiKey}&language=en-US`);
        ...
        box.innerHTML = `<iframe title="CineFlex trailer preview" src="https://www.youtube-nocookie.com/embed/${encodeURIComponent(trailer.key)}?autoplay=1&mute=1&controls=0&disablekb=1&loop=1&playlist=${encodeURIComponent(trailer.key)}&playsinline=1&modestbranding=1&rel=0&origin=${encodeURIComponent(location.origin)}" allow="autoplay; encrypted-media; picture-in-picture" referrerpolicy="strict-origin-when-cross-origin"></iframe>`;
  ...
  function applyHero(item) {
    ...
    // Keep the controls minimal, but autoplay a muted trailer preview when TMDB provides one.
    loadTrailer(item);
  }
