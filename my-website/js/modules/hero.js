(function () {
  ...
  let soundEnabled = sessionStorage.getItem('cineflex_trailer_sound') === 'on';
  window.CineFlexHeroTrailerPlaying = false;
  ...
  function enableTrailerSound() {
    soundEnabled = true;
    sessionStorage.setItem('cineflex_trailer_sound', 'on');
    try { player?.unMute?.(); player?.setVolume?.(65); } catch (_) {}
  }

  function installSoundUnlock() {
    const unlock = () => enableTrailerSound();
    ['pointerdown','touchstart','click','keydown'].forEach(type =>
      document.addEventListener(type, unlock, { once: true, capture: true, passive: type !== 'keydown' })
    );
  }
      ...
      try {
        ...
        player = new YT.Player(mount.id, {
          ...
          playerVars: {
            ...
            modestbranding: 1, rel: 0, iv_load_policy: 3, origin: location.origin, enablejsapi: 1
          },
          events: {
            onReady(event) {
              if (token !== trailerToken) return;
              try { if (soundEnabled) { event.target.unMute(); event.target.setVolume(65); } else { event.target.mute(); event.target.setVolume(0); } event.target.playVideo(); const frame=event.target.getIframe?.(); if(frame){frame.setAttribute('allow','autoplay; encrypted-media; fullscreen; picture-in-picture'); frame.setAttribute('playsinline','');} } catch (_) {}
            },
            onStateChange(event) {
              if (token !== trailerToken) return;
              if (event.data === YT.PlayerState.PLAYING) {
                trailerPlaying = true;
                window.CineFlexHeroTrailerPlaying = true;
                clearTimeout(rotateTimer);
                clearTimeout(trailerTimer);
