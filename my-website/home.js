...
// --- UPDATED DISPLAY CARDS WITH NETFLIX TOP 10 BADGE ---
function displayCards(data, containerId) {
  ...
  container.innerHTML = data.filter(i => i.poster_path).map((item, index) => {
    let trendingBadge = "";
    if (containerId === "netflix-movies-list" && index < 10) {
        trendingBadge = `<div class="netflix-badge">Top ${index + 1}</div>`;
    }

    return `
    <div class="card" style="position: relative;" tabindex="0" onclick='showDetails(${JSON.stringify(item).replace(/'/g, "&apos;")})'>
        ${trendingBadge}
