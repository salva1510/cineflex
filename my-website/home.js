...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      const reg = await navigator.serviceWorker.register('/service-worker.js?v=250.3', {
        updateViaCache: 'none'
      });
      await reg.update();
...
async function init() {
  try {
    const [trd, fil, kd] = await Promise.all([
      // Trending remains the source for the hero banner and Top 10 module.
      fetch(`${BASE_URL}/trending/all/day?api_key=${API_KEY}`).then(r => r.json()),
      fetch(`${BASE_URL}/discover/movie?api_key=${API_KEY}&region=PH&with_origin_country=PH&sort_by=popularity.desc`).then(r => r.json()),
      fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&with_original_language=ko&with_genres=18&sort_by=popularity.desc`).then(r => r.json())
    ]);

    trendingItems = Array.isArray(trd.results) ? trd.results : [];
    if (trendingItems.length > 0) setBanner(trendingItems[0]);

    displayCards(Array.isArray(fil.results) ? fil.results : [], "filipino-list");
    displayCards(Array.isArray(kd.results) ? kd.results : [], "kdrama-list");
