...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      const reg = await navigator.serviceWorker.register('/service-worker.js?v=250.1', {
        updateViaCache: 'none'
      });
      await reg.update();
...
async function init() {
  try {
    const [trd, fil, kd, kids, netflixMovies, horror] = await Promise.all([
      // Trending is retained only for the featured homepage banner.
