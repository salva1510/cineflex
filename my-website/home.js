...
// --- PLAYERS DOMAINS ---
const PEACHIFY_URL = "https://peachify.pro";

let currentItem = null;
let trendingItems = [];
let currentBannerIndex = 0;
// Build 304.5: expose live hero state to modular trailer/navigation controllers.
try {
  Object.defineProperty(window, "trendingItems", { configurable: true, get: () => trendingItems, set: value => { trendingItems = Array.isArray(value) ? value : []; } });
  Object.defineProperty(window, "currentBannerIndex", { configurable: true, get: () => currentBannerIndex, set: value => { currentBannerIndex = Number(value) || 0; } });
  window.API_KEY = API_KEY;
  window.BASE_URL = BASE_URL;
} catch (error) { console.warn("Hero state bridge unavailable", error); }
...
// --- BUILD 301.0: CENTRAL PLAY-ACTION AD CONTROLLER ---
// Ads are allowed only on Play Now and direct episode-card clicks.
function triggerPlaybackPopAd(action = "play-now") {
  return window.CineFlexPlayAds?.trigger(action) || false;
}

// Compatibility wrappers retained for existing Play Now buttons only.
function triggerPopUnder() { return false; }
function triggerPlayPopUnderForCurrentTitle() { return triggerPlaybackPopAd("play-now"); }
...
async function init() {
  ...
  updateContinueUI();
  await cfLoadPersonalizedRows();
  setupInfiniteScroll();
  document.body.classList.add('cf-home-ready');
}


const CF_RECOMMENDATION_CACHE_KEY = 'cineflex_recommendations_v1';
const CF_RECOMMENDATION_CACHE_MS = 6 * 60 * 60 * 1000;

function cfReadLocalArray(key) {
  try { const value = JSON.parse(localStorage.getItem(key) || '[]'); return Array.isArray(value) ? value : []; }
  catch (_) { return []; }
}

function cfUniqueItems(items, limit = 20) {
  const seen = new Set();
...
function cfRecommendationSignals() {
  const recent = cfReadLocalArray('cineflex_recent');
  const list = cfReadLocalArray('cineflex_watchlist');
  const source = [...recent.slice(0, 12), ...list.slice(0, 12)];
  const genres = new Map();
  source.forEach(item => (item.genre_ids || []).forEach(id => genres.set(id, (genres.get(id) || 0) + 1)));
  const topGenres = [...genres.entries()].sort((a,b) => b[1]-a[1]).slice(0,3).map(([id]) => id);
  return { recent, list, source, topGenres };
}

async function cfLoadPersonalizedRows(force = false) {
  if (window.CineFlexPlugins && !window.CineFlexPlugins.isEnabled('recommendations')) {
    document.getElementById('recommended-section')?.setAttribute('hidden', '');
    document.getElementById('because-watched-section')?.setAttribute('hidden', '');
    return;
  }
  const recommended = document.getElementById('recommended-list');
  if (!recommended) return;
  const signals = cfRecommendationSignals();
  const anchor = signals.recent[0] || signals.list[0] || null;
  try {
    if (!force) {
      const cached = JSON.parse(localStorage.getItem(CF_RECOMMENDATION_CACHE_KEY) || 'null');
      if (cached?.savedAt && Date.now() - cached.savedAt < CF_RECOMMENDATION_CACHE_MS && Array.isArray(cached.items)) {
        displayCards(cached.items, 'recommended-list');
        if (cached.becauseItems?.length) cfRenderBecauseWatched(cached.becauseItems, cached.anchorTitle);
        return;
      }
    }
    const type = anchor && (anchor.media_type === 'tv' || anchor.name || anchor.first_air_date) ? 'tv' : 'movie';
    const discoverType = signals.recent.filter(x => x.media_type === 'tv' || x.name).length > signals.recent.length / 2 ? 'tv' : 'movie';
    const genreQuery = signals.topGenres.length ? `&with_genres=${signals.topGenres.join(',')}` : '';
    const requests = [cfFetchJson(`${BASE_URL}/discover/${discoverType}?api_key=${API_KEY}&sort_by=popularity.desc&vote_count.gte=100${genreQuery}`)];
    if (anchor?.id) requests.push(cfFetchJson(`${BASE_URL}/${type}/${anchor.id}/recommendations?api_key=${API_KEY}`));
    const results = await Promise.allSettled(requests);
    const recommendedItems = results[0].status === 'fulfilled' ? (results[0].value.results || []) : (trendingItems || []);
    const watchedIds = new Set(signals.source.map(x => String(x.id)));
    const cleanRecommended = cfUniqueItems(recommendedItems.filter(x => !watchedIds.has(String(x.id))), 18);
    displayCards(cleanRecommended.length ? cleanRecommended : trendingItems, 'recommended-list');
    let becauseItems = [];
    if (results[1]?.status === 'fulfilled') {
      becauseItems = cfUniqueItems((results[1].value.results || []).filter(x => !watchedIds.has(String(x.id))), 18);
      cfRenderBecauseWatched(becauseItems, anchor.title || anchor.name || 'your last title');
    }
    localStorage.setItem(CF_RECOMMENDATION_CACHE_KEY, JSON.stringify({savedAt:Date.now(),items:cleanRecommended,becauseItems,anchorTitle:anchor?.title || anchor?.name || ''}));
  } catch (error) {
    console.warn('Personal recommendations unavailable:', error);
    displayCards(trendingItems || [], 'recommended-list');
  }
}

function cfRenderBecauseWatched(items, title) {
  const section = document.getElementById('because-watched-section');
  if (!section || !items?.length) { if (section) section.style.display='none'; return; }
  section.style.display = '';
  const heading = document.getElementById('because-watched-heading');
  if (heading) heading.textContent = `Because You Watched ${title || ''}`.trim();
  displayCards(items, 'because-watched-list');
}

window.cfRefreshRecommendations = function() {
  localStorage.removeItem(CF_RECOMMENDATION_CACHE_KEY);
  const recommended = document.getElementById('recommended-list');
  if (recommended) recommended.innerHTML = '<div class="cf-poster-skeleton"></div><div class="cf-poster-skeleton"></div><div class="cf-poster-skeleton"></div>';
  cfLoadPersonalizedRows(true);
};
...
function openPremiumWatchPage({ episode = 1, adAction = "play-now" } = {}) {
    if (!currentItem) return;
    ...
    const adTriggered = triggerPlaybackPopAd(adAction);
    // Keep the source page alive long enough for the provider tag to initialize.
    window.setTimeout(() => { window.location.href = destination; }, adTriggered ? 900 : 0);
}
...
function playSpecificEpisode(epNum, element) {
    ...
    window.currentTVState = currentTVState;
    openPremiumWatchPage({ episode: epNum, adAction: 'episode-click' });
}
...
// TV remote navigation is handled exclusively by cinematic-tv304.js (Build 304.1).
...
// Build 304.8: legacy automatic banner interval removed.
// js/modules/hero.js alone controls rotation and waits for trailer completion.
    ...
    if(main && !document.getElementById('cf-community-section')){
      ...
      sec.innerHTML = `
        ...
        <div class="cf-community-grid">
          ...
          <div class="cf-community-card"><span>Community activity</span><strong id="cf-card-activity-count">Live</strong><small>foundation for trending and admin stats</small></div>
        </div>
...
(function initPremiumHomepage2506(){
  ...
  function restartHero(){
    // Build 304.5: hero.js owns rotation so a banner never changes mid-trailer.
    clearInterval(heroTimer); drawHeroDots(); preloadHero(currentBannerIndex+1);
  }
...
// Build 300.7 plugin registrations
window.CineFlexPlugins?.register('ads', { trigger: triggerPlaybackPopAd });
window.CineFlexPlugins?.register('recommendations', { refresh: window.cfRefreshRecommendations, load: cfLoadPersonalizedRows });
